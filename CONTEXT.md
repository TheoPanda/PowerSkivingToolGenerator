...
② **几何**：外缘 = 齿圈谷底圆柱 r_root（派生只读，用户不可填）；前端面 = 前刀面平面整环延伸（[23]§3.3②「每区域前端面∥对应刀齿前刀面」在本项目单平面刀的退化：所有齿共用同一前刀面平面 → 整环同面）；**纯轴向直拉伸**（键槽必须笔直——现实装键约束，斜齿不做螺旋扭转，后端与刀齿的微小错缝预览级接受）；键槽朝刀具坐标 +X；直孔（主流盘形插齿刀形态）。
