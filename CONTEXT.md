...
② **几何（2026-08-31 两次实测返工后定稿）**：**碗形回转体**（用户图纸 4035100343「碗型斜齿车齿刀」锚定——真实车齿刀为碗形，无盘形）：内锥碗面（轮毂高 = 0.4643·B_total，图纸 13/28）+ 外壁 + 背平板 + 内孔壁；**碗口边缘沿谷底弧**（与齿圈内壁同线，消除直壁-弧形内壁接缝）；前端面 z 按槽位折叠（理想平面与「基准齿×z_t 纯旋转」整环错位十余毫米——「光滑透明椭球」根因）；**纯轴向直拉伸**（键槽笔直——现实装键约束，斜齿不做螺旋扭转）；键槽朝刀具坐标 +X、沿内孔壁全高贯通；直孔；**材质 = 硬质合金 carbide 预设**（用户指定）。键槽尺寸图纸锚定：31.743 孔 → 14×6.0（R1），覆盖插齿刀手册/GB-T 6132 近似值。
