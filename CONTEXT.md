...
**扩展 (2026-08-11)**: 原 `r_b <= r_f` 被 `solve_root_fillet` 拒绝（"无需圆角"），导致高齿数齿轮（z=82/z=60，r_b<r_f）指定 `root_fillet` 也画不出圆角。已扩展：搜索起点改为齿面从齿根圆起始（`xi_at_radius(r_b, r_f)`），齿面-齿根圆连接角处双切圆角仍可解；门控放宽为 `root_fillet` 即可。深齿根无解仍回退径向线。
