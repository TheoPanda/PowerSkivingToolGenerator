...
| 刀具整环 | Tool Ring | z_t 个单齿模型绕刀具轴的**同相位周向阵列**总成（K-3.1），`toolRing` 图层的几何内容，模块③ 的可视化交付形态。单齿本体已是沿导程螺旋扫掠的条带（自带扭转），周向纯旋转即拼成完整斜齿刀体，**无逐齿轴向错位**——2026-08-27 勘误：曾按 ΔZ_i=i·p_z·j_t 施加错位，用户实测证实会产生 p_z≈24mm 的齿间轴向间隙（蜗杆状弹簧），已回退。U13 约束：不得镜像拼环，必须完整阵列 |
| 预览级 | Preview Grade | 模块③ 几何交付的质量等级标签：闭合三角网伪实体（K-3.1，仅 GLB），非 OCCT BRep/STEP 制造级；亦不含刀体结构（K-3.2 内孔/键槽/刃带——设计书该节未回读 W9，刀具几何体面板 v1 明确禁入）。区别于未来制造级 ToolSolid |
