...
| 刀体 | Tool Body (Hub) | 齿圈以内的支承本体：**碗形回转体**（内锥碗面+外壁+背平板+内孔壁，用户图纸 4035100343「碗型斜齿车齿刀」形态），含内孔、键槽（沿内孔壁全高、笔直）、碗口边缘沿齿圈谷底弧与齿圈无缝拼合。渲染为硬质合金（与单齿同材质）。`toolBody` 图层的几何内容 |
...
② **几何（2026-08-31 三次实测迭代后定稿：v8/v4 圆柱求差）**：**光滑圆柱基体**——齿圈内壁的谷底弧花瓣由 **r_limit（= r_a − a，干涉物理上限）圆柱求差**修平（齿圈内壁 = r_limit 光滑圆，`build_tooth_loop` v8 求差底弧），刀体 = 同半径**圆柱**（`build_tool_body` v4），刀齿立于圆柱基体之上无缝一体；前端面为平面（z 取基准窗口相位中心），不做刀齿阶梯结构顺延；键槽朝刀具坐标 +X、沿内孔壁全高贯通且笔直（斜齿不做螺旋扭转）；直孔；**刀齿与刀体均渲染硬质合金 carbide**（用户指定「一体渲染」）。键槽尺寸图纸 4035100343 锚定：31.743 孔 → 14×6.0（R1），覆盖插齿刀手册/GB-T 6132 近似值。碗形回转体（图纸形态）经实测否决后退为候选升级（git 历史 4a4f304）。
