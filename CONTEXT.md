...
**扩展2 (2026-08-11)**: 低齿数齿轮（z<30，深下切 r_b−r_f>ρ）请求半径太小塞不下 → 方案 A 把半径放大到最小可行双切半径 ρ_min=(r_b²−r_f²)/(2r_f)·1.001（仍真双切圆角），`RootFillet.rho` 存实际半径，`rho_f_actual`/标注报实际。摆线齿根（方案 B/T13）仍留待后续。
