...
export type MaterialPreset = 'steel' | 'carbide' | 'generatrix' | 'rake' | 'edge'
...
export const MATERIAL_PRESETS: Record<MaterialPreset, MaterialPresetDef> = {
  ...
  // 后刀面/单齿（对齐现有 carbideMaterial 0x5a5854）
  ...
  // 前刀面 — 冰蓝 #E8F0F8
  rake: { color: 0xe8f0f8, roughness: 0.4, metalness: 0.3, transparent: true, opacity: 0.45 },
  // 刃形 — 深色 #1f2937（LineBasicMaterial，roughness/metalness 无效仅占位）
  edge: { color: 0x1f2937, roughness: 0.5, metalness: 0.0, transparent: false, opacity: 1.0 },
}
...
export const LAYER_VISUALS: Record<LayerId, LayerVisual> = {
  ...
  flank: { id: 'flank', label: '后刀面', kind: 'mesh', materialPreset: 'carbide', defaultOpacity: 0.6, doubleSide: true },
