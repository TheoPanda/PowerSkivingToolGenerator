...
import { TOOL_BODY_ID, TOOL_RING_ID } from '../three/layerPalette'
...
// ── 步骤3 图层预设（PRD §5.5 / ADR-020③；ADR-021 修订：保留刀具整环+刀体）──
// 切入瞬间一次性收拢为「完整的一把刀」可见——单留齿圈悬空反而显得缺件。
// watch 对 ref 只在值变化时触发 → 每次切入步骤3 都执行、停在步骤3 不重复；离开无动作。
// 预设不锁死：用户可再手动开任一层，直到下次切入才重新收拢。viewport 未登记时 useLayers 安全跳过。
const STEP3_KEEP_LAYERS: LayerId[] = [TOOL_RING_ID, TOOL_BODY_ID]

watch(currentStep, (step: number): void => {
  if (step === 3) useLayers().hideAllExcept(STEP3_KEEP_LAYERS)
})
