...
def _resolved_bore_keyway(**kw) -> ToolBodyResolved:
    base = dict(mounting="bore_keyway", d_pt=84.9, m_n=2.0, L=16.0, r_cut=25.0)
    base.update(kw)
...
class TestResolve:
    def test_defaults_bore(self):
        r = resolve_tool_body_params(
            mounting="bore", d_pt=84.9, m_n=2.0, L=16.0, r_cut=25.0
        )
    ...
    def test_hard_bore_edge_crosses_root(self):
        # φ100 档孔 44.443：r_bore=22.22 ≥ r_cut=22 → 硬拒
        with pytest.raises(ValueError, match="越求差圆"):
            resolve_tool_body_params(
                mounting="bore", d_pt=100.0, m_n=2.0, L=18.0, r_cut=22.0,
                d_bore=44.443,
            )

    def test_hard_keyway_bottom_crosses_root(self):
        # φ75 键槽底 r_bore+t1 = 15.8715+6.0 = 21.87 ≥ 18.5 → 硬拒（键槽底判据）
        with pytest.raises(ValueError, match="键槽底越求差圆"):
            _resolved_bore_keyway(r_cut=18.5)
    ...
    def test_invalid_mounting(self):
        with pytest.raises(ValueError, match="装夹形式"):
            resolve_tool_body_params(
                mounting="flange", d_pt=84.9, m_n=2.0, L=16.0, r_cut=25.0
            )
...
def _build(**kw):
    base = dict(mounting="bore_keyway", d_pt=84.9, m_n=2.0, L=12.0, B=12.0, r_cut=30.0)
    base.update(kw)
...
class TestToolBodyMesh:
    ...
    def test_volume_matches_bowl_estimate(self, mesh):
        ...
        vol /= 6.0
        r_cut, r_bore, B = 30.0, 31.743 / 2.0, 12.0
        # Cavalieri：圆柱直拉伸截面积恒定 → V = (π(R²−r²) − b·t1)·B（键槽矩形从环形扣除；
        # 口部月牙薄楔并入 void，欠 ~0.3% 落入容差）
        analytic = (math.pi * (r_cut**2 - r_bore**2) - 14.0 * 6.0) * B
        assert vol > 0
        assert abs(vol - analytic) / analytic < 5e-3  # 圆弦采样欠估 + 薄楔
    ...
    def test_rim_smooth_circle(self):
        """接缝回归锁（2026-08-31 用户裁决「圆柱求差」）：外壁 = r_cut 光滑圆.

        齿圈内壁已被同半径求差圆修平（花瓣退役）；刀体外壁必须是与求差面同半径的
        光滑圆柱——逐采样点半径恒等于 r_cut，无任何瓣状起伏。
        """
        spec, _ = _build()
        pos = spec.positions
        n = len(pos) // 3 // 4
    ...
    def test_front_face_flat_at_phase_center(self):
    ...
    def test_plain_bore_has_no_keyway(self):
        resolved = resolve_tool_body_params(
            mounting="bore", d_pt=84.9, m_n=2.0, L=12.0, r_cut=30.0
        )
    ...
    def test_degenerate_guard_r_root_le_bore(self):
        ...
        with pytest.raises(ValueError, match="≤ 孔半径"):
            build_tool_body(_RAKE, r_cut=15.0, resolved=resolved, theta_c=0.0)
