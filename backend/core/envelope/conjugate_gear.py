...
def _cap_arc_points(positions: list[float], va: int, vb: int, n_arc: int) -> list[list[float]]:
    """生成连接顶点 va→vb 的圆弧 n_arc 个点（绕 z 轴短弧；端点精确复制、中间点线性插值半径/角/z）.

    产形面桶形 → 齿顶/齿根半径随层变化，中间点半径 r(t)=r0+t·(r1−r0) 线性过渡，
    保证端点与侧刃面边界顶点坐标精确重合（无缝）。
    """
...
def _build_cap_face(
    ...
    返回 (positions_flat, indices)。每层弧 n_arc 个点（端点复制自侧刃面边界顶点），
    相邻层弧连成 n_arc−1 段 × 2 三角的直纹回转面。
    """
    ...
    for iz in range(n_z):
        ...
        arc = _cap_arc_points(positions, va, vb, n_arc)
        base = len(cap_pos) // 3
        for p in arc:
            cap_pos += p
        if prev_base >= 0:
...
def compute_conjugate_gear(
    ...
    *,
    ...
    n = n_vert // n_z  # 每层廓形点数（conjugate.py positions 行主序 index = iz·n + iu）

    top_pos, top_idx = _build_cap_face(single_pos, single_idx, n, n_z, "top", n_arc)
    root_pos, root_idx = _build_cap_face(single_pos, single_idx, n, n_z, "root", n_arc)
    ...
    # 绕刀具 z 轴阵列 z_t 份（旋转只作用 (x,y)，法向由合并后重算）
    ...
    normals = _compute_vertex_normals(all_pos, all_idx)
