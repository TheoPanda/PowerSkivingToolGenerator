"""模块②b 离散包络点云 + 产形面 — 设计书 K-2.9 纯数学.

由工件齿廓点云经 K-0.5 运动链扫掠得 m×n 点云（产形面，坐标 T），
并按规则网格三角化为 mesh（供 GLB 导出）。不依赖 OCCT。

K-2.10 投影 z=0 属刃形提取（edge.py），不在此模块——产形面 mesh 为
...
def extract_gap_points(p, n_points: int = 200) -> list[tuple[float, float]]:
    """从 GearParams 提取单齿槽廓形点（离散包络输入）.

    内齿轮 k_io=−1 用 tooth_gap_segments（齿槽 = 刀具齿的反包络源，ADR-017）；
    外齿轮 k_io=+1 用 single_tooth_segments（单齿）。MVP 端到端跑内齿轮。
    """
    from core.workpiece.profile import single_tooth_segments, tooth_gap_segments
...
def generate_envelope_cloud(
    ...
    *,
    ...
    Returns:
        GeneratrixCloud（cloud 含 z 分量；mesh 为原始点云直接三角化）
